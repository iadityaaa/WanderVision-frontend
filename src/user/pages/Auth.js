...
const Auth = () => {
  ...
  const { isLoading, error, sendRequest, clearError } = useHttpClient(); //A custom hook managing many things related to req
  ...
  const authSubmitHandler = async (event) => {
    //authSubmitHandler which fires whenever we authenticate(login/signup)
    ...
    if (isLoginMode) {
      ...
      try {
        //Since api is on different port we need to provide the complete path
        const formData = new FormData(); //A browser side api
  ...
  return (
    <React.Fragment>
      {/* Passing the error usestate as error prop to the ErrorModal comp */}
